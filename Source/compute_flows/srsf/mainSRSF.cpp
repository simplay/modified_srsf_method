...
int main(int argc, char **argv) {
	...
	if (argc == 5) {
		...
		if (bSEL != 0) {
			...
			printf ("Set Sel = 0 (Non-Rigid) for 10 parameters \n");
			return(0);
		...
		if (bSEL != 2) {
			...
			printf ("Set Sel = 2 (Rigid+NonRigid) for 11 parameters \n");
			return(0);
		...
		if (bSEL != 1) {
			...
			printf ("Set Sel = 1 (Rigid) for 7 parameters \n");
			return(0);
		...
		printf ("Number of parameter should be 4, 7, 10 or 11 \n");
		return(0);

	}

	// determine whether we are computing the forward or backward flow field
	flow_type_suff = (im_i < im_f)? "fw" : "bw";


	vector<string> sep = split(dataset_path, '/');
	String dataset_name = sep[sep.size()-1];
	cout << "ds name: " << dataset_name << endl;
	struct stat st = {0};
	String dirname = "./output/"+dataset_name;
	...
	char name[100];
	FileStorage SFlow;
	String flow_id = flow_type_suff + "_flow_" + std::to_string(im_i)+"_" + std::to_string(im_f);
	String output_flow_file_name = "./output/" + dataset_name + "/" + flow_id + ".xml";
	SFlow.open(output_flow_file_name, FileStorage::WRITE);
	...
	while (Bframe < 2) {
		...
		String cfn = dataset_path + std::to_string(im) + ".png";
		cout << "using color img: " << cfn << "\n";
		RGB = imread(cfn);
		String dfn = dataset_path + "depth/" + std::to_string(im) + ".png";
		cout << "using depth img: " << dfn << "\n";
		DEPTH = imread(dfn, CV_LOAD_IMAGE_UNCHANGED);
